...
export default function EditCourseForm({ courseData, courseId }: { courseData: Course, courseId: string }) {
  ...
  const [value, setValue] = useState<JSONContent | undefined>(courseData?.details)
          ...
          <CardContent>
            <div className="grid gap-6">
              ...
              <div className='grid gap-2'>
                <Label>Details</Label>
                <input type="hidden"
                  name={fieldsEdit.details.name}
                  key={fieldsEdit.details.name}
                  defaultValue={fieldsEdit.details.initialValue}
                  value={JSON.stringify(value)}
                />
                <TailwindEditor onChange={setValue} initialValue={value} />
                <p className="text-red-500 text-sm">{fieldsEdit.details.errors}</p>
              </div>
            </div>
          </CardContent>
